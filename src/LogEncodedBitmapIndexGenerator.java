 ...
 */
public class LogEncodedBitmapIndexGenerator {
	...
	public LogEncodedBitmapIndexGenerator(int initial_length_of_bit_index,
		...
		this.encodedValueMap = new HashMap<String, List<String>>();
	}

	public LogEncodedBitmapIndexGenerator(ArrayList<String> datacolumn,
	...
	public void Insert(String recordvalue) {
		...
		List<String> bitmapKeyArray = updateEncodeValueMap(recordvalue);
		boolean iterOne=true;
		for (String strKey : bitmapKeyArray) {
			if (iterOne && !strKey.equals(DELETE_BITMAP_INDEX_NAME)) {
				this.current_record_count++;
			}

			if (!this.data_dictionary.containsKey(strKey)) {
				...
				this.data_dictionary.put(strKey, bitstring);
				this.UpdateFirstBit(this.current_record_count - 1, strKey);
				// No need to minus as there may be next insert
				if (!strKey.equals(DELETE_BITMAP_INDEX_NAME)) {
			...
			if (this.current_record_count + INSTRUCTION_BIT_LENGTH >= this.data_dictionary
				...
				this.ExpandBitString(strKey);
			}
			// System.out
			// .println(hashKey
			// + ":"
			// + (data_dictionary.get(hashKey).length -
			// INSTRUCTION_BIT_LENGTH));
			this.data_dictionary.get(strKey)[INSTRUCTION_BIT_LENGTH
					+ current_record_count - 1] = true;

			this.UpdateOneBitCount(strKey, true);
			iterOne=false;
		}
...
//		Print();
	...
	private List<String> updateEncodeValueMap(String recoredValue) {
		// int
		// prevBitmapCount=(int)Math.ceil(Math.log(cardinality)/Math.log(2));
		if (!encodedValueMap.containsKey(recoredValue)) {
			int cardinality = encodedValueMap.size();			
			cardinality++;
			// if(cardinality==1){
			// cardinality++;
			// }
			// int reqBitmapCount = (int) Math.ceil(Math.log(cardinality)
			// / Math.log(2));
			// System.out.println(reqBitmapCount+":"+Math.log(cardinality)+":"+cardinality);
			ArrayList<String> ans = new ArrayList<>();
			// String[] ans = new String[reqBitmapCount];
			String strCardinality = Integer.toBinaryString(cardinality);
			// System.out.println(recoredValue + "|" + cardinality + "|"
			// + strCardinality);
			for (int i = 1; i <= strCardinality.length(); i++) {
				if (strCardinality.charAt(strCardinality.length() - i) == '1') {
					ans.add("B" + i);
				}
			}
			encodedValueMap.put(recoredValue, ans);
			return ans;
		}
		return encodedValueMap.get(recoredValue);
	}

	private List<String> getEncodeValueMapArray(String recordValue) {
		return encodedValueMap.get(recordValue);
	}
	...
	public List<Integer> getIdArray(String recordValue) {
		...
		List<String> encodedKeyArray = getEncodeValueMapArray(recordValue);
		Map<String, boolean[]> boolArrays = new HashMap<>();
		for (String key : data_dictionary.keySet()) {
			boolArrays.put(key, data_dictionary.get(key));
		}
		List<Integer> ans = new ArrayList<Integer>();

		if (encodedKeyArray != null) {
			for (int i = INSTRUCTION_BIT_LENGTH; i < current_record_count; i++) {
				boolean a = true;
				for (String allKeys : data_dictionary.keySet()) {
					if (encodedKeyArray.contains(allKeys)) {
						// System.out.print((i+1)+",");
						if (boolArrays.get(allKeys).length >= current_record_count) {
							if (!boolArrays.get(allKeys)[i]) {
								a = false;
								break;
							}
						}
